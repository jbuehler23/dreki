//! Hello 3D — A lit rotating cube on a ground plane.
//!
//! Demonstrates Camera3d, Mesh3d, PBR Material, and lighting.

use dreki::prelude::*;

fn main() {
    env_logger::init();

    App::new()
        .add_plugins(DefaultPlugins)
        .set_title("dreki — hello 3d")
        .insert_resource(ClearColor([0.1, 0.1, 0.15, 1.0]))
        .insert_resource(AmbientLight {
            color: [1.0, 1.0, 1.0],
            intensity: 0.05,
        })
        .add_startup_system(setup)
        .add_system(rotate_cube)
        .add_system(move_camera)
        .run();
}

fn setup(world: &mut World) {
    // Camera looking at the origin from above and behind
    world.spawn((
        Transform::from_xyz(0.0, 3.0, 6.0).looking_at(Vec3::ZERO, Vec3::Y),
        Camera3d::default(),
    ));

    // Red cube (slightly metallic)
    world.spawn((
        Transform::from_xyz(0.0, 0.75, 0.0),
        cube_mesh(),
        Material {
            base_color: [0.8, 0.1, 0.1, 1.0],
            metallic: 0.3,
            roughness: 0.4,
            ..Default::default()
        },
    ));

    // Ground plane (large, gray, fully rough)
    world.spawn((
        Transform::from_xyz(0.0, 0.0, 0.0).with_scale(10.0),
        plane_mesh(),
        Material {
            base_color: [0.4, 0.4, 0.4, 1.0],
            metallic: 0.0,
            roughness: 0.9,
            ..Default::default()
        },
    ));

    // Blue plastic sphere (non-metallic so diffuse shading is visible)
    world.spawn((
        Transform::from_xyz(2.0, 0.5, -1.0),
        sphere_mesh(),
        Material {
            base_color: [0.2, 0.4, 0.9, 1.0],
            metallic: 0.0,
            roughness: 0.4,
            ..Default::default()
        },
    ));

    // Directional light (sun)
    world.spawn_one(DirectionalLight {
        direction: Vec3::new(-0.5, -1.0, -0.3),
        color: [1.0, 0.98, 0.95],
        intensity: 1.5,
    });

    // Point light (warm, near the cube)
    world.spawn((
        Transform::from_xyz(2.0, 2.5, 2.0),
        PointLight {
            color: [1.0, 0.8, 0.6],
            intensity: 5.0,
            radius: 10.0,
        },
    ));
}

/// Rotate the cube each frame.
fn rotate_cube(world: &mut World) {
    let dt = world.resource::<Time>().delta_secs();
    let cube_handle = cube_mesh().mesh;

    // Rotate all cubes (entities with Mesh3d that use the built-in cube)
    world.query::<(&mut Transform, &Mesh3d)>(|_entity, (transform, mesh3d)| {
        if mesh3d.mesh == cube_handle {
            transform.rotation *= Quat::from_rotation_y(0.8 * dt);
        }
    });
}

/// Simple WASD camera movement.
fn move_camera(world: &mut World) {
    let dt = world.resource::<Time>().delta_secs();
    let input = world.resource::<Input<KeyCode>>();
    let speed = 5.0;

    let mut movement = Vec3::ZERO;
    if input.pressed(KeyCode::KeyW) { movement.z -= 1.0; }
    if input.pressed(KeyCode::KeyS) { movement.z += 1.0; }
    if input.pressed(KeyCode::KeyA) { movement.x -= 1.0; }
    if input.pressed(KeyCode::KeyD) { movement.x += 1.0; }
    if input.pressed(KeyCode::Space) { movement.y += 1.0; }
    if input.pressed(KeyCode::ShiftLeft) { movement.y -= 1.0; }

    if movement == Vec3::ZERO {
        return;
    }
    let movement = movement.normalize() * speed * dt;

    world.query_single::<(&mut Transform,), Camera3d>(|_entity, (transform,)| {
        // Move in camera-local space (forward = -Z)
        let forward = transform.rotation * Vec3::NEG_Z;
        let right = transform.rotation * Vec3::X;
        let up = Vec3::Y;
        transform.translation += forward * movement.z + right * movement.x + up * movement.y;
    });
}
