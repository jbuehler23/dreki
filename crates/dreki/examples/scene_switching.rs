...
fn main() {
    ...
    Game::new("dreki — scene switching (Enter game, Escape menu)")
...
fn setup(ctx: &mut Context) {
    ...
    load_scene_tagged(&mut ctx.world, &registry, &menu_scene_data(), "menu");
}
...
fn switch_to_game(ctx: &mut Context) {
    ...
    let registry = make_registry();
    switch_scene(&mut ctx.world, &registry, "menu", &game_scene_data(), "game");
...
fn switch_to_menu(ctx: &mut Context) {
    ...
    let registry = make_registry();
    switch_scene(&mut ctx.world, &registry, "game", &menu_scene_data(), "menu");
