...
pub fn physics_step_3d(world: &mut World) {
...
/// Plugin: inserts a [`PhysicsWorld3d`] resource and registers [`physics_step_3d`].
///
/// Optionally enables debug collider wireframes (requires `render3d` feature).
...
impl Physics3dPlugin {
    /// Create a new plugin with default gravity `(0, -9.81, 0)` and debug colliders enabled.
    pub fn new() -> Self {
        Self {
            gravity: Vec3::new(0.0, -9.81, 0.0),
    ...
    /// Disable debug collider wireframes.
    pub fn without_debug_colliders(mut self) -> Self {
        self.debug_colliders = false;
        self
    }
}

impl Default for Physics3dPlugin {
    fn default() -> Self {
        Self::new()
    }
}

impl crate::app::Plugin for Physics3dPlugin {
    fn build(&self, app: &mut crate::app::App) {
        app.world
            .insert_resource(PhysicsWorld3d::new().with_gravity(self.gravity));
        app.systems.add_system(physics_step_3d);
        #[cfg(feature = "render3d")]
        if self.debug_colliders {
            app.world
                .insert_resource(crate::render3d::DebugColliders3d::default());
        }
    }
}
