...
pub(crate) fn collect_camera(
    ...
    world.query_single::<(&Transform, &Camera3d), Camera3d>(|_entity, (transform, cam)| {
        ...
        camera_uniform.camera_pos = transform.translation.to_array();
    });
...
pub(crate) fn collect_lights(world: &mut World) -> LightUniform {
    ...
    let mut count = 0u32;
    world.query::<(&Transform, &PointLight)>(|_entity, (transform, light)| {
...
pub(crate) fn collect_draw_calls(world: &mut World) -> Vec<DrawCall> {
    ...
    world.query::<(&Transform, &Mesh3d, &Material)>(|_entity, (transform, mesh3d, material)| {
    ...
    world.query::<(&Transform, &Shape3d)>(|_entity, (transform, shape)| {
        let shape_scale = shape.shape_scale();
        let model = transform.matrix() * glam::Mat4::from_scale(shape_scale);
        let normal_matrix = model.inverse().transpose();
