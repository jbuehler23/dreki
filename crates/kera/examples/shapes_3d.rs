//! 3D Shapes — spheres, cuboids, cylinders, and planes with PBR materials.

use kera::prelude::*;

fn main() {
    env_logger::init();

    App::new()
        .add_plugins(DefaultPlugins)
        .set_title("kera — 3d shapes")
        .insert_resource(ClearColor([0.08, 0.08, 0.12, 1.0]))
        .insert_resource(AmbientLight {
            color: [1.0, 1.0, 1.0],
            intensity: 0.08,
        })
        .add_startup_system(setup)
        .add_system(rotate_shapes)
        .add_system(move_camera)
        .run();
}

/// Marker for shapes that should spin.
struct Spinning;

fn setup(world: &mut World) {
    // Camera
    world.spawn((
        Transform::from_xyz(0.0, 5.0, 12.0).looking_at(Vec3::new(0.0, 1.0, 0.0), Vec3::Y),
        Camera3d::default(),
    ));

    // Lights
    world.spawn_one(DirectionalLight {
        direction: Vec3::new(-0.4, -1.0, -0.3),
        color: [1.0, 0.98, 0.95],
        intensity: 1.2,
    });
    world.spawn((
        Transform::from_xyz(4.0, 4.0, 4.0),
        PointLight {
            color: [1.0, 0.85, 0.7],
            intensity: 8.0,
            radius: 15.0,
        },
    ));
    world.spawn((
        Transform::from_xyz(-4.0, 3.0, -2.0),
        PointLight {
            color: [0.6, 0.8, 1.0],
            intensity: 6.0,
            radius: 12.0,
        },
    ));

    // Ground plane
    world.spawn((
        Transform::from_xyz(0.0, 0.0, 0.0),
        Shape3d::plane(20.0, 20.0)
            .color([0.25, 0.25, 0.28, 1.0])
            .roughness(0.9),
    ));

    // ── Back row: material showcase (varying metallic + roughness) ──────
    // Y = radius so spheres sit on the floor (unit sphere radius 0.5 × scale 2.0 = radius 1.0).

    // Matte red sphere (plastic)
    world.spawn((
        Transform::from_xyz(-4.0, 1.01, -3.0),
        Shape3d::sphere(1.0)
            .color([0.9, 0.15, 0.15, 1.0])
            .roughness(0.8),
    ));

    // Semi-glossy green sphere
    world.spawn((
        Transform::from_xyz(-1.5, 1.01, -3.0),
        Shape3d::sphere(1.0)
            .color([0.15, 0.8, 0.2, 1.0])
            .roughness(0.4),
    ));

    // Shiny blue sphere
    world.spawn((
        Transform::from_xyz(1.5, 1.01, -3.0),
        Shape3d::sphere(1.0)
            .color([0.2, 0.3, 0.9, 1.0])
            .roughness(0.1),
    ));

    // Gold metallic sphere
    world.spawn((
        Transform::from_xyz(4.0, 1.01, -3.0),
        Shape3d::sphere(1.0)
            .color([1.0, 0.766, 0.336, 1.0])
            .metallic(1.0)
            .roughness(0.3),
    ));

    // ── Front row: different shape types ────────────────────────────────
    // Y = half-height + 0.01 so shapes sit just above the floor (avoids z-fighting).

    // Red spinning cube (half-height = 0.75)
    world.spawn((
        Transform::from_xyz(-3.0, 0.76, 2.0),
        Shape3d::cuboid(1.5, 1.5, 1.5)
            .color([0.85, 0.2, 0.2, 1.0])
            .metallic(0.3)
            .roughness(0.4),
        Spinning,
    ));

    // Tall cyan cylinder (half-height = 1.25)
    world.spawn((
        Transform::from_xyz(-0.5, 1.26, 2.0),
        Shape3d::cylinder(0.6, 2.5)
            .color([0.1, 0.8, 0.8, 1.0])
            .roughness(0.3),
    ));

    // Wide flat purple cuboid (table-like, half-height = 0.4)
    world.spawn((
        Transform::from_xyz(2.5, 0.41, 2.0),
        Shape3d::cuboid(2.0, 0.8, 1.2)
            .color([0.6, 0.2, 0.8, 1.0])
            .roughness(0.5),
    ));

    // Small spinning metallic sphere on top of the table
    world.spawn((
        Transform::from_xyz(2.5, 1.22, 2.0),
        Shape3d::sphere(0.4)
            .color([0.95, 0.95, 0.95, 1.0])
            .metallic(1.0)
            .roughness(0.05),
        Spinning,
    ));

    // Orange spinning cylinder (half-height = 0.75)
    world.spawn((
        Transform::from_xyz(5.0, 0.76, 2.0),
        Shape3d::cylinder(0.5, 1.5)
            .color([1.0, 0.5, 0.1, 1.0])
            .metallic(0.5)
            .roughness(0.35),
        Spinning,
    ));
}

fn rotate_shapes(world: &mut World) {
    let dt = world.resource::<Time>().delta_secs();

    world.query_filtered::<(&mut Transform,), Spinning>(|_entity, (transform,)| {
        transform.rotation *= Quat::from_rotation_y(0.8 * dt);
    });
}

fn move_camera(world: &mut World) {
    let dt = world.resource::<Time>().delta_secs();
    let input = world.resource::<Input<KeyCode>>();
    let speed = 5.0;

    let mut movement = Vec3::ZERO;
    if input.pressed(KeyCode::KeyW) { movement.z -= 1.0; }
    if input.pressed(KeyCode::KeyS) { movement.z += 1.0; }
    if input.pressed(KeyCode::KeyA) { movement.x -= 1.0; }
    if input.pressed(KeyCode::KeyD) { movement.x += 1.0; }
    if input.pressed(KeyCode::Space) { movement.y += 1.0; }
    if input.pressed(KeyCode::ShiftLeft) { movement.y -= 1.0; }

    if movement == Vec3::ZERO {
        return;
    }
    let movement = movement.normalize() * speed * dt;

    world.query_single::<(&mut Transform,), Camera3d>(|_entity, (transform,)| {
        let forward = transform.rotation * Vec3::NEG_Z;
        let right = transform.rotation * Vec3::X;
        let up = Vec3::Y;
        transform.translation += forward * movement.z + right * movement.x + up * movement.y;
    });
}
