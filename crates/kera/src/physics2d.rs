...
pub fn physics_step_2d(world: &mut World) {
...
/// Plugin: inserts a [`PhysicsWorld2d`] resource and registers [`physics_step_2d`].
///
/// Optionally enables debug collider wireframes (requires `render2d` feature).
...
impl Physics2dPlugin {
    /// Create a new plugin with default gravity `(0, -9.81)` and debug colliders enabled.
    pub fn new() -> Self {
        Self {
            gravity: Vec2::new(0.0, -9.81),
...
impl crate::app::Plugin for Physics2dPlugin {
    fn build(&self, app: &mut crate::app::App) {
        app.world
            .insert_resource(PhysicsWorld2d::new().with_gravity(self.gravity));
        app.systems.add_system(physics_step_2d);
        #[cfg(feature = "render2d")]
        if self.debug_colliders {
            app.world
                .insert_resource(crate::render2d::DebugColliders2d::default());
        }
    }
}
