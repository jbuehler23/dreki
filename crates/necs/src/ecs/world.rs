...
impl World {
    ...
    pub fn entity_name(&self, entity: Entity) -> Option<&str> {
        self.names_reverse.get(&entity.index()).map(|s| s.as_str())
    }

    /// Get the tags on an entity.
    pub fn entity_tags(&self, entity: Entity) -> Vec<String> {
        self.entity_tags
            .get(&entity.index())
            .cloned()
            .unwrap_or_default()
    ...
    /// Get all named entities as (entity, name) pairs.
    pub fn named_entities(&self) -> Vec<(Entity, &str)> {
        self.names
            .iter()
            .map(|(name, &entity)| (entity, name.as_str()))
            .collect()
    }
    ...
    /// Get the type names of all components on an entity.
    pub fn entity_component_names(&self, entity: Entity) -> Vec<&str> {
        let Some(loc) = self.entity_locations.get(&entity.index) else {
            return Vec::new();
        };
        let Some(arch) = self.archetypes.get(&loc.archetype_key) else {
            return Vec::new();
        };
        arch.type_name_map.values().map(|s| s.as_ref()).collect()
    }
    ...
    pub fn insert<T: 'static + Send + Sync>(&mut self, entity: Entity, component: T) {
        ...
        let old_type_names = old_arch.type_name_map.clone();
        ...
        // Carry over type names from the old archetype + add the new type.
        for (type_id, name) in old_type_names {
            new_arch.type_name_map.entry(type_id).or_insert(name);
        }
        new_arch.type_name_map.entry(tid).or_insert(std::any::type_name::<T>());
    ...
    pub fn remove<T: 'static + Send + Sync>(&mut self, entity: Entity) -> bool {
        ...
        let new_row = new_arch.entities.len();
        new_arch.entities.push(entity);
        // Carry over type names (excluding the removed type).
        for (type_id, name) in old_type_names {
            if type_id != tid {
                new_arch.type_name_map.entry(type_id).or_insert(name);
            }
...
/// short name (e.g. `necs::math::Transform` → `Transform`).
#[cfg(feature = "diagnostics")]
...
mod tests {
    ...
    #[test]
    fn entity_name_returns_name() {
        let mut world = World::new();
        let e = world.spawn((Marker,));
        world.name_entity(e, "hero");

        assert_eq!(world.entity_name(e), Some("hero"));
    }

    #[test]
    fn entity_name_returns_none_for_unnamed() {
        let mut world = World::new();
        let e = world.spawn((Marker,));

        assert_eq!(world.entity_name(e), None);
    }

    #[test]
    fn entity_tags_returns_all_tags() {
        let mut world = World::new();
        let e = world.spawn((Marker,));
        world.tag(e, "enemy");
        world.tag(e, "boss");

        let tags = world.entity_tags(e);
        assert_eq!(tags.len(), 2);
        assert!(tags.contains(&"enemy".to_string()));
        assert!(tags.contains(&"boss".to_string()));
    }

    #[test]
    fn entity_tags_empty_for_untagged() {
        let mut world = World::new();
        let e = world.spawn((Marker,));

        assert!(world.entity_tags(e).is_empty());
    }

    #[test]
    fn named_entities_lists_all() {
        let mut world = World::new();
        let e1 = world.spawn((Position { x: 0.0, y: 0.0 },));
        let e2 = world.spawn((Position { x: 1.0, y: 1.0 },));
        let _e3 = world.spawn((Position { x: 2.0, y: 2.0 },)); // unnamed

        world.name_entity(e1, "alpha");
        world.name_entity(e2, "beta");

        let named = world.named_entities();
        ...
        let entities: Vec<Entity> = named.iter().map(|(e, _)| *e).collect();
        assert!(entities.contains(&e1));
        assert!(entities.contains(&e2));
    }

    #[test]
    fn all_entities_returns_every_alive_entity() {
        let mut world = World::new();
        let e1 = world.spawn((Marker,));
        let e2 = world.spawn((Position { x: 0.0, y: 0.0 },));
        let e3 = world.spawn((Position { x: 1.0, y: 1.0 }, Velocity { dx: 0.0, dy: 0.0 }));
    ...
    #[test]
    fn all_entities_excludes_despawned() {
        let mut world = World::new();
        let e1 = world.spawn((Marker,));
        let e2 = world.spawn((Marker,));
    ...
    #[test]
    fn entity_component_names_lists_types() {
        let mut world = World::new();
        let e = world.spawn((
        ...
        let names = world.entity_component_names(e);
        assert_eq!(names.len(), 2);
        // Names are fully-qualified, check they contain the short name.
        assert!(names.iter().any(|n| n.contains("Position")));
        assert!(names.iter().any(|n| n.contains("Velocity")));
    }

    #[test]
    fn entity_component_names_empty_for_dead_entity() {
        let mut world = World::new();
        let e = world.spawn((Marker,));
        world.despawn(e);

        assert!(world.entity_component_names(e).is_empty());
    }

    #[test]
    fn entity_component_names_updates_after_insert() {
        let mut world = World::new();
        let e = world.spawn((Position { x: 0.0, y: 0.0 },));

        assert_eq!(world.entity_component_names(e).len(), 1);
    ...
    #[test]
    fn entity_component_names_updates_after_remove() {
        let mut world = World::new();
        let e = world.spawn((
            Position { x: 0.0, y: 0.0 },
        ...
        assert_eq!(world.entity_component_names(e).len(), 2);
    ...
    #[test]
    fn entity_name_cleaned_up_on_despawn() {
        let mut world = World::new();
        let e = world.spawn((Marker,));
        world.name_entity(e, "temp");

        assert_eq!(world.entity_name(e), Some("temp"));
        world.despawn(e);
        // After despawn, entity_name should return None (entity is dead).
        assert_eq!(world.entity_name(e), None);
    }

    #[test]
    fn entity_tags_cleaned_up_on_despawn() {
        let mut world = World::new();
        let e = world.spawn((Marker,));
        world.tag(e, "level1");

        assert_eq!(world.entity_tags(e).len(), 1);
        world.despawn(e);
        // After despawn, entity_tags returns empty.
        assert!(world.entity_tags(e).is_empty());
    }
}
